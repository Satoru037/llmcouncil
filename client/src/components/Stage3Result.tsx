...
export const Stage3Result: React.FC<Stage3ResultProps> = ({
	...
	const [activeStage2Tab, setActiveStage2Tab] = useState<string>(
		state.selectedModels[0] || "",
	);
	...
	const saveConversation = async () => {
		try {
			const response = await fetch(`${API_URL}/api/conversations`, {
				...
				body: JSON.stringify({
					title:
						state.question.length > 50
							? state.question.substring(0, 50) + "..."
							: state.question,
					question: state.question,
					data: state,
				}),
		...
		return (
			<div className='flex flex-col items-center justify-center py-20'>
				...
				<p className='text-gray-500 mt-2'>
					Synthesizing insights from {state.selectedModels.length} models
				</p>
			</div>
		);
	}
					...
					<div className='p-6 min-h-[200px]'>
						<div className='prose prose-sm max-w-none text-gray-700'>
							<ReactMarkdown remarkPlugins={[remarkGfm]}>
								{state.stage1Responses.find((r) => r.model === activeTab)
									?.response || ""}
							</ReactMarkdown>
						</div>
					</div>
						...
						{state.stage2Reviews.map((review) => {
							...
							const extractedRanking = (() => {
								...
								for (const line of lines) {
									const rankMatch = line
										.trim()
										.match(/(\d+)\.\s*(Response [A-Z])/i);
									if (rankMatch) {
							...
							return (
								<div
									...
									<div className='space-y-6'>
										...
										{/* Aggregate Ranking */}
										{state.stage3Result &&
											state.stage3Result.aggregate_rankings.length > 0 && (
												<div className='bg-blue-50 p-6 rounded-xl border border-blue-100'>
													<h5 className='font-bold text-blue-700 text-sm mb-4 flex items-center gap-2'>
														Aggregate Rankings
													</h5>
													<p className='text-xs text-gray-500 mb-4'>
														Combined results across all peer evaluations (lower
														score is better):
													</p>
													<div className='space-y-2'>
														{state.stage3Result.aggregate_rankings.map(
															(rank, i) => {
																const modelIndex =
																	state.stage1Responses.findIndex(
																		(r) => r.model === rank.model,
																	);
																const label =
																	modelIndex !== -1
																		? `Response ${String.fromCharCode(65 + modelIndex)}`
																		: rank.model;

																return (
																	<div
																		key={rank.model}
																		className='flex items-center bg-white p-3 rounded-lg border border-blue-100 shadow-sm'>
			...
			<div className='space-y-4' ref={stage3Ref}>
				...
				<div className='bg-white p-8 rounded-lg shadow-lg border border-green-200 relative overflow-hidden'>
					...
					<div className='flex justify-between items-start mb-6'>
						<div>
							<h2 className='text-lg font-semibold text-green-800 flex items-center gap-2'>
								Chairman:{" "}
								<span className='font-mono text-sm bg-green-100 px-2 py-1 rounded text-green-700'>
									{state.stage3Result?.chairman_model || "gemini-3-pro-preview"}
								</span>
							</h2>
						</div>
					...
					<div className='prose prose-blue max-w-none text-gray-800 leading-relaxed'>
						<ReactMarkdown remarkPlugins={[remarkGfm]}>
							{state.stage3Result.final_answer}
						</ReactMarkdown>
					</div>
				</div>
			</div>
