...
function App() {
  ...
  useEffect(() => {
    ...
    if (lastId) {
      loadConversation({ id: lastId, title: 'Loading...', created_at: '', data: undefined } as SavedConversation);
    }
  ...
  const loadConversation = useCallback(async (saved: SavedConversation) => {
    try {
  ...
  const confirmDeleteConversation = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/conversations/${conversationToDelete}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        setSavedConversations(prev => prev.filter(c => c.id !== conversationToDelete));
        
        if (conversation.id === conversationToDelete) {
          startNewSession();
        }
        
        if (conversationToDelete === localStorage.getItem('lastActiveConversationId')) {
          localStorage.removeItem('lastActiveConversationId');
        }
      }
    } catch (err) {
